...
public class TempDiffLambdaRMLNN extends ReinforcementLearner
{

	protected static final File saveLocation=new File("/home/c/workspace/Reinforcement Machine Learning/SavedNeuralNets/neuralNet");
	...
	public TempDiffLambdaRMLNN(Environment enviroment, 
			NeuralEGreedyPolicy policy,
			State startState,
			int numberEpisodes,
			double stepSize,
			double discountRate,
	...
	public void learn() 
	{
		for(int numberEpisodesCompleted=0; numberEpisodesCompleted<numberEpisodes; numberEpisodesCompleted++)
		{
			...
			{
				actions++;
				reward=enviroment.takeAction(bestAction, this);
			...
			double diff=reward-getActionStateValue(currentState, bestAction);
			setActionStateEligibility(currentState, bestAction,
					getActionStateEligibility(currentState, bestAction)+1);
			...
			((NeuralEGreedyPolicy)policy).trainNN();
