...
public class BackPropGradientDescentStopBelowEval extends BackPropGradientDescent
{
	
	protected double maxEval;
	protected EvaluationFunction evaluationFunction;
	
	public BackPropGradientDescentStopBelowEval(int batchSize, int epochs, double learningRate, 
			...
			ArrayRealVector[] desiredOutputs, CostFunction costFunction) 
	{
		...
		for(int currentEpoch=0; currentEpoch<epochs && eval>maxEval; currentEpoch++)
		{
			...
			for(int sampleInd=0; sampleInd<inputs.length; sampleInd++)
			{
				...
				totalCost+=costFunction.getCost(inputs[sampleInd], networkOutput, desiredOutputs[sampleInd]);
			}
			totalCost/=inputs.length;
			...
			{
				ArrayRealVector networkOutput=network.getOutput(inputs[sampleInd]);
				if(desiredOutputs[sampleInd].getL1Norm()==0.0)
				{
					int u=0;
				}
			...
			eval=evaluationFunction.getEval(network);
