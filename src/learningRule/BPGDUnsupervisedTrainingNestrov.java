...
public class BPGDUnsupervisedTrainingNestrov extends MPBackPropGradientDescentNestrov
{
	
	protected int unsupervisedEpochs;
	
	public BPGDUnsupervisedTrainingNestrov(int batchSize, int epochs, int unsupervisedEpochs, float learningRate, float momentumFactor) 
	{
			...
			for(int netLayerIndex=0; netLayerIndex<network.layers[layerIndex].length; netLayerIndex++)
			{
				...
				if(layerIndex<network.layers.length-1)
				{
					if(savedInputs!=null)
					{
						...
						MPBackPropGradientDescent bpgd=new MPBackPropGradientDescent(1000, unsupervisedEpochs, 0.01f);
