...
public class MPBackPropGradientDescentNestrov extends MPLearningRule 
{
	...
	public MPBackPropGradientDescentNestrov(int batchSize, int epochs, float learningRate, float momentumFactor)
	{
	...
	{		
		...
		for(int currentEpoch=0; currentEpoch<epochs; currentEpoch++)
		{
			...
			for(int sampleInd=0; sampleInd<randomizedInputs.size(); sampleInd+=batchSize)
			{
				...
				Object[][] pdss=new Object[Math.min(batchSize, randomizedInputs.size()-sampleInd)][];
				for(int batchInd=0; batchInd<Math.min(batchSize, randomizedInputs.size()-sampleInd); batchInd++)
				{
			...
			if((currentEpoch+1)%5==0)
			{
				...
				for(int sampleInd=0; sampleInd<inputs.length; sampleInd++)
				{
					...
					len=networkOutput[0].getLen();
					for(Matrix matrix: networkOutput)
					{
						matrix.clear();
					}
				}
